...
![](p8105_hw3_zw2899_files/figure-gfm/unnamed-chunk-14-1.png)<!-- -->

-   Below is a two-panel plot showing (i) tmax vs tmin for the full
    dataset (note that a scatterplot may not be the best option);
    and (ii) make a plot showing the distribution of snowfall values
    greater than 0 and less than 100 separately by year.

Alternative to a scatterplot, we plot tmin and tmax across years
together and observe how one changes with the other over years.

``` r
tmax_tmin_full_data = cleaned_ny_noaa %>%
  filter(!is.na(tmin) & !is.na(tmax)) %>%
  mutate(day_365 = yday(date_full)) %>%
  pivot_longer(cols = tmax:tmin, names_to = "tmax_or_tmin", values_to = "temp") %>%
  ggplot(aes(x = day_365, y = temp, color = tmax_or_tmin, group = interaction(year, tmax_or_tmin))) +
...
![](p8105_hw3_zw2899_files/figure-gfm/unnamed-chunk-15-1.png)<!-- --> We
can see that in general, the maximum temperature and the minimum
...
![](p8105_hw3_zw2899_files/figure-gfm/unnamed-chunk-16-1.png)<!-- -->
